...
SUMO_GENERATE_OKTA_PROPERTIES=${SUMO_GENERATE_OKTA_PROPERTIES:=true}
...
SUMO_ENABLE_SCRIPTS=${SUMO_ENABLE_SCRIPTS:=true}
...
generate_okta_properties_file() {
    # Read values from ENV variables and place them in collector/config/collector.properties file
    declare -A SUPPORTED_OPTIONS
    SUPPORTED_OPTIONS=(
        ["SUMO_OKTA_KEY"]="api_token"
        ["SUMO_OKTA_URL"]="okta_org_url"
    )
    OKTA_PROPERTIES=""
    ...
    do
        value=${!key}
        if [ -n "${value}" ]; then
            OKTA_PROPERTIES="${OKTA_PROPERTIES}${SUPPORTED_OPTIONS[$key]}=${value}\n"
        fi
    done

    if [ -n "${OKTA_PROPERTIES}" ]; then
        echo -e ${OKTA_PROPERTIES} >> /sumojanus-okta/conf/sumologic.properties
    fi
}
